...
- Config and management made made easier for flexible production deployments with [.env.template](.env.template).
...
- Document and expose in easy to configure way some lesser-used features like BASE_URL and BASE_PATH for deployments behind proxies with paths (config automatically propagated into [nginx config](./infra/opencti/reverse-proxy/templates/default.conf.template)).
